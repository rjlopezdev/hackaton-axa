    ...
    if login == True:
        ...
        while seguro_contratado == False:
            ...
            if categoria in polizas:
                ...
                filtro_especialidad = filtro_especialidad[filtro_especialidad['CP'] == np.int64(cp)]
                ...
                if filtro_especialidad.empty:
                    ...
                    while not 's' in si_no:
                        if i >= filtro_especialidad.shape[0]:
                            print("Lo siento, no quedan más citas disponibles")
                            no_reservado = True
                            break
                        else:
                            cita_asignada = filtro_especialidad.iloc[i]
                            i = i + 1
                            print(f"¿Podría asistir el día {cita_asignada['Dia_Cita']} a las {cita_asignada['Hora_Cita']} en {cita_asignada['Dirección']}?")
                            si_no = input("").lower()
